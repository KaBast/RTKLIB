                  ...
                  const nav_t *nav)
{
	...
	if (stat != SOLQ_NONE)
	{
		if (AmbiguityFunction(rtk,stat,obs,nu,rs,dts,svh,nav,opt,dt,sat,iu,ir,ns,nf,n,y,e,azel,nr))
		{
	...
	//Ambiguity Function Ausgleichung -- Kay Bastian
	else if (stat == SOLQ_AMB)
	{
		for (i = 0; i < 3; i++)
		{
			//Speicherung der berechneten Koordinaten
			rtk->sol.rr[i] = rtk->xa[i];
		}
	}
    else {
...
/*Ambiguity Function Ausgleichung -- Kay Bastian*/
extern int AmbiguityFunction(rtk_t* rtk, const int stat, const obsd_t* obs, int nu, const double* rs,
							  ...
							  int nr)
{
	...
	int MAXKONV = 20, MAXGEWICHTE = 20;  //Wert der die maximale Schleifendurchgänge vorgibt
	double KONVGRENZE = 0.001;	//Abbruchkriterium der Ausgleichung
	...
	for (konvAusgl = 0; konvAusgl < MAXKONV; konvAusgl++)
	{
		...
		if (!zdres(0, obs, nu, rs, dts, svh, nav, Ambx, opt, 0, y, e, azel)) {
		...
		nv = AmbFunc_ddres(rtk, nav, dt, Ambx, Pp, sat, y, e, azel, iu, ir, ns, v, H, R, vflg, LamList, MessList);
		if (nv < 1)
		{
		...
		for (ii = 0; ii < nv; ii++)
		{
			...
			if (MessList[ii] > (nf - 1)) continue;
			//Richtungskosinus
			ax = -H[0 + ii * rtk->nx];
			ay = -H[1 + ii * rtk->nx];
			az = -H[2 + ii * rtk->nx];
			/*Ambiguity Function Modell Verbesserungsgleichungen*/
			v1 = cos(2.0 * PI / LamList[ii] * (-v[ii] - (ax * dx + ay * dy + az * dz)));
			v2 = sin(2.0 * PI / LamList[ii] * (-v[ii] - (ax * dx + ay * dy + az * dz)));
			/*Realteil = Verbesserungsgleichung v1*/
			A[0 + Matii * 6] = 2.0 * PI / LamList[ii] * v2 * ax;
			A[1 + Matii * 6] = 2.0 * PI / LamList[ii] * v2 * ay;
			A[2 + Matii * 6] = 2.0 * PI / LamList[ii] * v2 * az;
			l[0 + Matii * 2] = v1 - 1.0;
			/*Imaginärteil = Verbesserungsgleichung v2*/
			A[3 + Matii * 6] = -2.0 * PI / LamList[ii] * v1 * ax;
			A[4 + Matii * 6] = -2.0 * PI / LamList[ii] * v1 * ay;
			A[5 + Matii * 6] = -2.0 * PI / LamList[ii] * v1 * az;
			l[1 + Matii * 2] = v2 - 0.0;
			Matii++;
		}
		...
		{ 
			...
			PGewicht[hh + 2 * Matii * hh] = 1.0/(fabs(verb[hh]) + 1.0);
		}
...
/*Ambiguity Function -- Kay Bastian*/
static int AmbFunc_ddres(rtk_t* rtk, const nav_t* nav, double dt, const double* x,
	const double* P, const int* sat, double* y, double* e,
	double* azel, const int* iu, const int* ir, int ns, double* v,
...
/* undifferenced phase/code residual for satellite ---------------------------*/
static void AmbFunc_zdres_sat(int base, double r, const obsd_t* obs, const nav_t* nav,
	const double* azel, const double* dant,
	const prcopt_t* opt, double* y)
{
	const double* lam = nav->lam[obs->sat - 1];
...
/*Ambiguity Function -- Kay Bastian*/
static int AmbFunc_zdres(int base, const obsd_t* obs, int n, const double* rs,
	const double* dts, const int* svh, const nav_t* nav,
	const double* rr, const prcopt_t* opt, int index, double* y,
	double* e, double* azel)
{
	...
	for (i = 0; i < n; i++) {
		...
		/* undifferenced phase/code residual for satellite */
		AmbFunc_zdres_sat(base, r, obs + i, nav, azel + i * 2, dant, opt, y + i * nf * 2);
	}
	trace(4, "rr_=%.3f %.3f %.3f\n", rr_[0], rr_[1], rr_[2]);
	trace(4, "pos=%.9f %.9f %.3f\n", pos[0] * R2D, pos[1] * R2D, pos[2]);
	for (i = 0; i < n; i++) {
